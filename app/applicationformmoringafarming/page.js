...
export default function MoringaFarmingForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    businessActivity: 'Moringa Farming', industryType: 'Service', projectCost: '', bankName: '', accountNumber: '', ifscCode: '', bankBranch: '',
  ...
  useEffect(() => {
        ...
        try {
          ...
          if (pending.formType === 'moringa-farming' && pending.orderId) {
            ...
            if (verifyData?.order?.order_status === 'PAID') {
              ...
              const saveRes = await fetch('/api/forms/moringa-farming/save', {
                method: 'POST',
    ...
    if (savedData) {
      try {
        ...
        const savedStep = localStorage.getItem('moringa_farming_step');
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (formData.aadhar.length === 12 || formData.mobile.length === 10) {
        try {
          const res = await fetch('/api/forms/moringa-farming/check', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleManualSubmit = async () => {
    ...
    try {
      const checkRes = await fetch('/api/forms/moringa-farming/check', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ aadhar: formData.aadhar, mobile: formData.mobile })
      });
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between relative min-h-[80px]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none px-20 text-center">
            <h1 className="text-xl md:text-3xl font-black uppercase tracking-tight text-white leading-none mb-1 pointer-events-auto">
              Moringa Farming <span className="text-[#DEB887]">Application Form</span>
            </h1>
